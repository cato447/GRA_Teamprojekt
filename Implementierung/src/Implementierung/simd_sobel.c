...
void simd_sobel_graysc(uint8_t *img_in, size_t width, size_t height, uint8_t *img_out) {
    if (width * height >= 16 + 1 + 1 && height >= 1) {
        ...
        for (; i < width * (height - 1) - 1; ++i) {
            uint8_t upperLeft = *(img_in + i - width - 1);
            uint8_t upper = *(img_in + i - width);
            uint8_t upperRight = *(img_in + i - width + 1);
            uint8_t left = *(img_in + i - 1);
            uint8_t right = *(img_in + i + 1);
            uint8_t lowerLeft = *(img_in + i + width - 1);
            uint8_t lower = *(img_in + i + width);
            uint8_t lowerRight = *(img_in + i + width + 1);
...
__m128i applyFilter(__m128i upperLeft, __m128i upper, __m128i upperRight, __m128i left, __m128i right, __m128i lowerLeft, __m128i lower, __m128i lowerRight, __m128i comparer, __m128i zeroEvenBytesMask);

__attribute__((__always_inline__, __hot__)) inline
void computeSIMDSobel(uint8_t *img_in, size_t i, size_t width, uint8_t *img_out, __m128i zeroEvenBytesMask,
                      __m128i comparer) {
    ...
    __m128i result = applyFilter(upperLeft, upper, upperRight, left, right, lowerLeft, lower, lowerRight, comparer, zeroEvenBytesMask);
    __m128i result2 = applyFilter(upperLeft2, upper2, upperRight2, left2, right2, lowerLeft2, lower2, lowerRight2, comparer, zeroEvenBytesMask);
...
__attribute__((__always_inline__, __hot__)) inline
void computeSIMDSobel_graysc(uint8_t *img_in, size_t i, size_t width, uint8_t *img_out, __m128i zeroEvenBytesMask,
                      __m128i comparer) {
    __m128i upperLeft = _mm_loadu_si128((const __m128i *) (img_in + i - width - 1));
    __m128i upper = _mm_loadu_si128((const __m128i *) (img_in + i - width));
    __m128i upperRight = _mm_loadu_si128((const __m128i *) (img_in + i - width + 1));
    __m128i left = _mm_loadu_si128((const __m128i *) (img_in + i - 1));
    __m128i right = _mm_loadu_si128((const __m128i *) (img_in + i + 1));
    __m128i lowerLeft = _mm_loadu_si128((const __m128i *) (img_in + i + width - 1));
    __m128i lower = _mm_loadu_si128((const __m128i *) (img_in + i + width));
    __m128i lowerRight = _mm_loadu_si128((const __m128i *) (img_in + i + width + 1));

    __m128i upperLeft2 = _mm_loadu_si128((const __m128i *) (img_in + i - width - 1 + 1));
    __m128i upper2 = _mm_loadu_si128((const __m128i *) (img_in + i - width + 1));
    __m128i upperRight2 = _mm_loadu_si128((const __m128i *) (img_in + i - width + 1 + 1));
    __m128i left2 = _mm_loadu_si128((const __m128i *) (img_in + i - 1 + 1));
    __m128i right2 = _mm_loadu_si128((const __m128i *) (img_in + i + 1 + 1));
    __m128i lowerLeft2 = _mm_loadu_si128((const __m128i *) (img_in + i + width - 1 + 1));
    __m128i lower2 = _mm_loadu_si128((const __m128i *) (img_in + i + width + 1));
    __m128i lowerRight2 = _mm_loadu_si128((const __m128i *) (img_in + i + width + 1 + 1));
...
__m128i applyFilter(__m128i upperLeft, __m128i upper, __m128i upperRight, __m128i left, __m128i right, __m128i lowerLeft, __m128i lower, __m128i lowerRight, __m128i comparer, __m128i zeroEvenBytesMask) {
    ...
    */
    ...
    //This handles all values < 255.
    return _mm_or_si128(result, _mm_and_si128(sum, zeroEvenBytesMask));
}