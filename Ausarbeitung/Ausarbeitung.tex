...
Die Basisversion (Version 0) übersetzt die mathematische Definition des Sobel-Filters aus \ref{sec:math-def} schlicht in C-Code (mit einer kleinen Anpassung, siehe \ref{sec:vergleichsimplementierung}).
...
Die Vergleichsimplementierung ist eine naive Implementierung des Sobel-Filter Algorithmus und orientiert sich, wie bereits erwähnt, sehr stark an der mathematischen Definition aus \ref{sec:math-def}.
...
Da die mathematische Defintion, wie in \ref{sec:vergleichsimplementierung} beschrieben, ohne weitere Optimierungen umgesetzt wurde, ist deren Übereinstimmung unschwer zu überprüfen.
...
Eine weitere Optimierung, die bei der Arbeit mit Bildmaterial natürlich naheliegt, ist die Verwendung der
